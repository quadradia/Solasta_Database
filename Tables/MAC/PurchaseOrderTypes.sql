...
FROM sys.tables
WHERE name = 'PurchaseOrderTypes' AND schema_id = SCHEMA_ID('MAC'))
BEGIN
	CREATE TABLE [MAC].[PurchaseOrderTypes]
	(
		[PurchaseOrderTypeID] [int] IDENTITY(1,1) NOT NULL,
		...
		CONSTRAINT [PK_PurchaseOrderTypes] PRIMARY KEY CLUSTERED
(
...
IF NOT EXISTS (SELECT *
FROM sys.objects
WHERE name = 'DF_PurchaseOrderTypes_ModifiedById' AND type = 'D')
ALTER TABLE [MAC].[PurchaseOrderTypes] ADD  CONSTRAINT [DF_PurchaseOrderTypes_ModifiedById]  DEFAULT ((0)) FOR [ModifiedById]
GO

IF NOT EXISTS (SELECT *
FROM sys.objects
WHERE name = 'DF_PurchaseOrderTypes_CreatedDate' AND type = 'D')
...
IF NOT EXISTS (SELECT *
FROM sys.objects
WHERE name = 'DF_PurchaseOrderTypes_CreatedById' AND type = 'D')
ALTER TABLE [MAC].[PurchaseOrderTypes] ADD  CONSTRAINT [DF_PurchaseOrderTypes_CreatedById]  DEFAULT ((0)) FOR [CreatedById]
GO

IF NOT EXISTS (SELECT *
FROM sys.objects
WHERE name = 'DF_PurchaseOrderTypes_DEX_ROW_TS' AND type = 'D')
