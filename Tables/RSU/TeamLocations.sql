...
FROM sys.tables
WHERE name = 'TeamLocations' AND schema_id = SCHEMA_ID('RSU'))
BEGIN
...
IF NOT EXISTS (SELECT *
FROM sys.objects
WHERE name = 'DF_TeamLocations_DEX_ROW_TS' AND type = 'D')
...
ALTER TABLE [RSU].[TeamLocations]  WITH CHECK ADD  CONSTRAINT [FK_TeamLocations_DealerTenants] FOREIGN KEY([DealerTenantId])
REFERENCES [ACC].[DealerTenants] ([DealerTenantID])
GO

IF NOT EXISTS (SELECT *
FROM sys.foreign_keys
WHERE name = 'FK_TeamLocations_PoliticalStates')
...
IF NOT EXISTS (SELECT *
FROM sys.foreign_keys
WHERE name = 'FK_TeamLocations_PoliticalTimeZones')
...
IF NOT EXISTS (SELECT *
FROM sys.foreign_keys
WHERE name = 'FK_TeamLocations_Seasons')
...
ALTER TABLE [RSU].[TeamLocations] CHECK CONSTRAINT [FK_TeamLocations_DealerTenants]
GO
