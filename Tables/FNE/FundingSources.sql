...
FROM sys.tables
WHERE name = 'FundingSources' AND schema_id = SCHEMA_ID('FNE'))
BEGIN
...
IF NOT EXISTS (SELECT *
FROM sys.objects
WHERE name = 'DF_FundingSources_DEX_ROW_TS' AND type = 'D')
...
ALTER TABLE [FNE].[FundingSources]  WITH CHECK ADD  CONSTRAINT [FK_FundingSources_DealerTenants] FOREIGN KEY([DealerTenantId])
REFERENCES [ACC].[DealerTenants] ([DealerTenantID])
GO

IF NOT EXISTS (SELECT *
FROM sys.foreign_keys
WHERE name = 'FK_FundingSources_FundingSourceTypes')
...
ALTER TABLE [FNE].[FundingSources] CHECK CONSTRAINT [FK_FundingSources_DealerTenants]
GO
