 ...
 * Author:
 * Created: 2026-05-04
 * Dependencies: AFL.Affiliates, ACC.DealerTenants
 ******************************************************************************/

IF NOT EXISTS (SELECT *
FROM sys.tables
WHERE name = 'AffiliateCampaigns' AND schema_id = SCHEMA_ID('AFL'))
BEGIN
...
IF NOT EXISTS (SELECT *
FROM sys.objects
WHERE name = 'DF_AffiliateCampaigns_ModifiedById' AND type = 'D')
...
IF NOT EXISTS (SELECT *
FROM sys.objects
WHERE name = 'DF_AffiliateCampaigns_CreatedDate' AND type = 'D')
...
IF NOT EXISTS (SELECT *
FROM sys.objects
WHERE name = 'DF_AffiliateCampaigns_CreatedById' AND type = 'D')
...
IF NOT EXISTS (SELECT *
FROM sys.objects
WHERE name = 'DF_AffiliateCampaigns_DEX_ROW_TS' AND type = 'D')
...
WHERE name = 'FK_AffiliateCampaigns_Affiliates')
...
ALTER TABLE [AFL].[AffiliateCampaigns]  WITH CHECK ADD  CONSTRAINT [FK_AffiliateCampaigns_DealerTenants] FOREIGN KEY([DealerTenantId])
...
ALTER TABLE [AFL].[AffiliateCampaigns] CHECK CONSTRAINT [FK_AffiliateCampaigns_DealerTenants]
GO
