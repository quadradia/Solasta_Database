DROP PROCEDURE IF EXISTS [RSU].[spAutoGen_UserResourceImages_CREATE];
GO


/**********************************************************************************************************************
**		Name: [RSU].[spAutoGen_UserResourceImages_CREATE]
**		Desc: Autogenerated SPROC to CREATE a row for [RSU].[UserResourceImages]
**		Auth: ANDRES E. SOSA
**		Date: 10/02/2015 (UTC)
***********************************************************************************************************************/
CREATE   Procedure [RSU].[spAutoGen_UserResourceImages_CREATE]
(
		@UserResourceId INT
...
BEGIN
	 ...
	 * INITIALIZE
	 **************/
	 DECLARE @UserResourceImageID INT
	 ...
	 * CHECK ACCESS LEVEL
	 *********************/
	 IF (NOT EXISTS(SELECT * FROM [GEN].fxGetAccessLevel('CREATE','RSU','UserResourceImages') AS AL WHERE (AL.CreateAccessId > 0))) BEGIN
		--** Get ACL Information
		--SELECT @ACLUserID = UserId, @UserGUID = @UserGUID, @CreatedAccessId = AL.CreateAccessId  FROM [GEN].fxGetAccessLevel('CREATE','RSU','UserResourceImages') AS AL WHERE (AL.CreateAccessId > 0)
	 ...
	 **************/
	 SELECT * FROM RSU.UserResourceImages
		WHERE
		(UserResourceImageID = SCOPE_IDENTITY());
