DROP PROCEDURE IF EXISTS [RSU].[spAutoGen_UserTypeGroups_CREATE];
GO


/**********************************************************************************************************************
**		Name: [RSU].[spAutoGen_UserTypeGroups_CREATE]
**		Desc: Autogenerated SPROC to CREATE a row for [RSU].[UserTypeGroups]
**		Auth: ANDRES E. SOSA
**		Date: 10/02/2015 (UTC)
***********************************************************************************************************************/
CREATE   Procedure [RSU].[spAutoGen_UserTypeGroups_CREATE]
(
...
BEGIN
	 ...
	 * CHECK ACCESS LEVEL
	 *********************/
	 IF (NOT EXISTS(SELECT * FROM [GEN].fxGetAccessLevel('CREATE','RSU','UserTypeGroups') AS AL WHERE (AL.CreateAccessId > 0))) BEGIN
		--** Get ACL Information
		--SELECT @ACLUserID = UserId, @UserGUID = @UserGUID, @CreatedAccessId = AL.CreateAccessId  FROM [GEN].fxGetAccessLevel('CREATE','RSU','UserTypeGroups') AS AL WHERE (AL.CreateAccessId > 0)
	 ...
	 * CREATE ROW
	 ************/
	 INSERT INTO RSU.UserTypeGroups (
		[UserTypeGroupID]
		, [UserTypeGroupName]
		, [IsActive]
		, [ModifiedById]
