DROP PROCEDURE IF EXISTS [RSU].[spAutoGen_SeasonSummers_CREATE];
GO


/**********************************************************************************************************************
**		Name: [RSU].[spAutoGen_SeasonSummers_CREATE]
**		Desc: Autogenerated SPROC to CREATE a row for [RSU].[SeasonSummers]
**		Auth: ANDRES E. SOSA
**		Date: 10/02/2015 (UTC)
***********************************************************************************************************************/
CREATE   Procedure [RSU].[spAutoGen_SeasonSummers_CREATE]
(
...
BEGIN
	 ...
	 * CHECK ACCESS LEVEL
	 *********************/
	 IF (NOT EXISTS(SELECT * FROM [GEN].fxGetAccessLevel('CREATE','RSU','SeasonSummers') AS AL WHERE (AL.CreateAccessId > 0))) BEGIN
		--** Get ACL Information
		--SELECT @ACLUserID = UserId, @UserGUID = @UserGUID, @CreatedAccessId = AL.CreateAccessId  FROM [GEN].fxGetAccessLevel('CREATE','RSU','SeasonSummers') AS AL WHERE (AL.CreateAccessId > 0)
