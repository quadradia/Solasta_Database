DROP PROCEDURE IF EXISTS [RSU].[spAutoGen_UserEmployeeTypes_CREATE];
GO


/**********************************************************************************************************************
**		Name: [RSU].[spAutoGen_UserEmployeeTypes_CREATE]
**		Desc: Autogenerated SPROC to CREATE a row for [RSU].[UserEmployeeTypes]
**		Auth: ANDRES E. SOSA
**		Date: 10/02/2015 (UTC)
***********************************************************************************************************************/
CREATE   Procedure [RSU].[spAutoGen_UserEmployeeTypes_CREATE]
(
...
BEGIN
	 ...
	 * CHECK ACCESS LEVEL
	 *********************/
	 IF (NOT EXISTS(SELECT * FROM [GEN].fxGetAccessLevel('CREATE','RSU','UserEmployeeTypes') AS AL WHERE (AL.CreateAccessId > 0))) BEGIN
		--** Get ACL Information
		--SELECT @ACLUserID = UserId, @UserGUID = @UserGUID, @CreatedAccessId = AL.CreateAccessId  FROM [GEN].fxGetAccessLevel('CREATE','RSU','UserEmployeeTypes') AS AL WHERE (AL.CreateAccessId > 0)
	 ...
	 * CREATE ROW
	 ************/
	 INSERT INTO RSU.UserEmployeeTypes (
		[UserEmployeeTypeID]
		, [UserEmployeeTypeName]
		, [IsActive]
		, [CreatedById]
		, [ModifiedById]
	 )VALUES (
